...
function ClienteAutoCompleteSetValue(select, id) {
    if (id != null) {
        $.ajax({
            type: 'GET',
            url: caminhoWebSite + 'Funcionario/GetPesquisa?Id=' + id,
            async: false,
            success: function (data) {
                // create the option and append to Select2
                var option = new Option((data.length > 0 ? data[0].Matricula + ' - ' + data[0].Descricao : ""), (data.length > 0 ? data[0].Id : null), true, true);
